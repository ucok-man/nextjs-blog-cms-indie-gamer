  ...
  return (
    <nav className="max-w-screen-xl mx-auto sticky top-0 z-50 bg-brand-green-400 p-4 shadow-brand-y-lg">
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/"
          className="text-2xl font-black text-brand-white-200 bg-black p-2 border-4 border-brand-white-200 hover:-translate-y-brand-lg hover:shadow-brand-y-lg z-50"
        >
          <span className="flex">
          ...
          render={(item, idx) => (
            <Link key={idx} href={item.link} passHref className="">
              <Button
                variant="link"
                className={cn(
                  "text-lg uppercase font-bold border-4 p-5 m-0 bg-yellow-300",
                  pathname === item.link && "shadow-brand-br-md scale-x-110"
                )}
              >
                <span>{item.name}</span>
              </Button>
  ...
  return (
    <div className="lg:hidden">
      ...
      >
        ...
        >
          <Icons.close className="size-10 border-4 shadow-brand-br-md hover:translate-x-brand-md hover:shadow-none bg-brand-green-400" />
        </div>
          ...
          <Iterate
            ...
            render={(item, idx) => (
              <Link key={idx} href={item.link} passHref className="">
                <Button
                  variant="link"
                  className="text-3xl uppercase py-8 font-semibold border-4 shadow-brand-br-md hover:translate-x-brand-md hover:shadow-none min-w-[400px] bg-yellow-300"
                >
